...
def gc(base_dir: Path, dry_run: bool) -> int:
    ...
    if not cas_dir.exists():
        print(json.dumps({"ok": True, "deleted": 0, "note": "no cas/ directory"}))
        return 0
    ...
    if dry_run:
        print(json.dumps({"ok": True, "would_delete": [str(p) for p in to_delete]}, indent=2))
        return 0
    ...
    for p in to_delete:
        try:
            ...
            pass
    print(json.dumps({"ok": True, "deleted": len(to_delete)}, indent=2))
    return 0
