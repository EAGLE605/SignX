...
def register_modules():
    ...
    try:
        ...
        print(f"⚠️ Engineering module not available: {e}")
    ...
    try:
        ...
        print(f"⚠️ Workflow module not available: {e}")
    
    try:
        from modules import documents
        app.include_router(documents.router)
    except ImportError as e:
        print(f"⚠️ Documents module not available: {e}")
        
    try:
        from modules import quoting
        app.include_router(quoting.router)
    except ImportError as e:
        print(f"⚠️ Quoting module not available: {e}")
    
    print("=" * 60)
    print(f"✅ Platform ready with {len(registry.modules)} module(s)")
