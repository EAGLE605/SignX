...
async def create_shapes_table(conn):
    ...
    print("[OK] Created custom_sign_sections table")
...
async def import_aisc_data(conn):
    ...
    if records:
        ...
        for i in range(0, len(values), batch_size):
            ...
            if (i + batch_size) % 500 == 0:
                print(f"  Inserted {min(i + batch_size, len(values))}/{len(values)} shapes...")
    
    print(f"[OK] Imported {len(records)} AISC shapes")
    ...
    if a1085_file.exists():
        ...
        if a1085_labels:
            ...
            """, a1085_labels)
            print(f"[OK] Marked {len(a1085_labels)} shapes as ASTM A1085")
...
async def add_custom_sign_sections(conn):
    ...
    print(f"[OK] Added {len(custom_sections)} custom sign sections")
...
async def create_material_cost_table(conn):
    ...
    print("[OK] Created material cost tracking table")
...
async def create_sign_specific_views(conn):
    ...
    print("[OK] Created sign-specific views")
...
async def main():
    ...
    print("=" * 60)
    print("AISC SHAPES DATABASE IMPORT FOR APEX")
    print("=" * 60)
    
    # Check if files exist
    if not ENGINEERING_PATH.exists():
        print(f"ERROR: Path not found: {ENGINEERING_PATH}")
        print("Please ensure H:\\BOT TRAINING\\Engineering exists")
        return
    
    main_file = ENGINEERING_PATH / "aisc-shapes-database-v16.0.xlsx"
    if not main_file.exists():
        print(f"ERROR: AISC database not found: {main_file}")
        return
    ...
    try:
        ...
        print(f"Total shapes: {count}")
        print(f"HSS shapes: {hss_count}")
        print(f"A1085 HSS shapes: {a1085_count}")
        print("=" * 60)
        ...
        for row in result:
            print(f"  {row['designation']}: {row['weight_plf']} lb/ft, Sx={row['sx_in3']} in³")
