...
def main():
    ...
    if not Path(csv_path).exists():
        print(f"ERROR: AISC CSV not found at {csv_path}")
        print("Download from: https://www.aisc.org/resources/shapes-database/")
        sys.exit(1)

    # Load and normalize
    print(f"Loading AISC CSV: {csv_path}")
    raw = load_aisc_csv(csv_path)
    normalized = [normalize_row(r) for r in raw]
    print(f"Loaded {len(normalized)} sections")
    ...
    print(f"✓ Inserted {len(normalized)} pole sections into database")
