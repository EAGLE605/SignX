...
def main():
    """Main execution"""
    print("\n" + "="*70)
    print("EAGLE SIGN - GEMINI KNOWLEDGE BASE GENERATOR v2.0")
    print("Document-Centric Approach")
    print("="*70 + "\n")
    
    # Verify source directory
    if not DOC_ROOT.exists():
        print(f"✗ ERROR: Source directory not found: {DOC_ROOT}")
        print(f"  Please verify the BOT TRAINING directory location")
        input("\nPress Enter to exit...")
        sys.exit(1)
    
    # Step 1: Scan directory tree
    print("Step 1: Scanning BOT TRAINING directory...")
    print(f"Source: {DOC_ROOT}\n")
    
    documents = scan_directory_tree(DOC_ROOT)
    print(f"✓ Found {len(documents)} documents\n")
    
    if len(documents) == 0:
        print("✗ No documents found! Check directory structure")
        input("\nPress Enter to exit...")
        sys.exit(1)
    
    # Step 2: Categorize documents
    print("Step 2: Categorizing documents...")
    ...
    for category, docs in sorted(categorized.items(), key=lambda x: -len(x[1])):
        ...
        print(f"  {category.replace('_', ' ').title():20} {len(docs):4} docs  - {desc}")
    print()
    
    # Step 3: Generate HTML documents
    print("Step 3: Generating HTML summaries...")
...
"""
    ...
    print(f"✅ Master index: INDEX.html")
    
    if skipped:
        print(f"\n⚠ Skipped {len(skipped)} files (see details above)")
    
    print(f"\n📋 NEXT STEPS:")
    print(f"  1. Open INDEX.html in your browser to review")
    print(f"  2. Visit https://aistudio.google.com")
    print(f"  3. Create new corpus: 'eagle_sign_knowledge_base'")
    print(f"  4. Upload all HTML files (by category folders)")
    print(f"  5. Test with domain-specific queries")
    
    print(f"\n💡 CATEGORIES GENERATED:")
    for category in sorted(categorized.keys()):
        print(f"  • {category.replace('_', ' ').title()}: {len(categorized[category])} documents")
    
    print(f"\n🚀 This corpus contains your CURATED knowledge base!")
    print(f"   Much better than trying to extract from work orders.")
