...
async def test_shape_counts(conn):
    ...
    for row in type_counts:
        print(f"    {row['type']:15s}: {row['count']:4d} shapes")
...
async def test_a1085_flags(conn):
    ...
    for ex in examples:
        print(f"    {ex['aisc_manual_label']:20s} W={ex['w']:.1f} lb/ft, D={ex['d']:.2f} in")
...
async def test_eagle_sign_queries(conn):
    ...
    if len(results) > 0:
        ...
        for r in results:
            print(f"    {r['aisc_manual_label']:20s} W={r['w']:.1f} lb/ft, Ix={r['ix']:.1f} in⁴")
    else:
    ...
    if len(results) > 0:
        ...
        print(f"    (Material cost @ $0.90/lb)")
        for r in results:
            print(f"    {r['aisc_manual_label']:20s} ${r['cost_per_ft']:.2f}/ft, Ix={r['ix']:.1f} in⁴")
    else:
...
async def main():
    ...
    try:
        ...
        if passed == total:
            ...
            print(f"{Colors.GREEN}{'ALL TESTS PASSED'.center(60)}{Colors.RESET}")
            print(f"{Colors.GREEN}AISC database is fully functional for Eagle Sign projects{Colors.RESET}")
            print(f"{Colors.GREEN}{'='*60}{Colors.RESET}\n")
            return True
        else:
            print(f"\n{Colors.RED}{'='*60}{Colors.RESET}")
            print(f"{Colors.RED}{f'{total - passed} TESTS FAILED'.center(60)}{Colors.RESET}")
            print(f"{Colors.RED}Review errors above and fix issues{Colors.RESET}")
